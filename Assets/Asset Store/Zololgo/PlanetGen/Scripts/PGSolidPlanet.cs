...
public class PGSolidPlanet : MonoBehaviour {
	...
	void RenderPlanet (bool forceRender) {
		...
		equirectangularTex.anisoLevel != anisotropicLevel || forceRender) {
			...
			// Adjust code here
			RenderTexture tempRT = RenderTexture.GetTemporary(equirectangularTex.width, equirectangularTex.height, 0, RenderTextureFormat.ARGB32);
			Graphics.Blit(renderTex, tempRT, mat);
			Graphics.Blit(tempRT, renderTex); // Copy it back to source if needed
